...
function journeyReducer(store, action){
    ...
    switch(action.type){
        ...
        case 'NEXT_STEP':
            return {
                ...store,
                currentStep: store.currentStep < store.totalSteps-1 ? ++store.currentStep : store.totalSteps-1,
            }
        case 'UPDATE_STEP':

            const steps = [...store.steps].filter((step, i) => step.stepId !== action.stepId);
...
function JourneyActionsCreators(journeyStore, dispatch){
    ...
    const addValidator = (stepId, validator) => {
        ...
        if(typeof(validator) !== 'function') throw Error(`"validator" must be a function. Recieved typeof: ${typeof(validator)}`)
    ...
    const _getStep = (stepId) => {
        ...
        const step = journeyStore.steps.find(step => step.stepId === currentStepId);
        const {validator} = validators.current?.find(step => step.stepId === currentStepId); 
        return {
            step, 
            validator
        }; 
...
export function JourneyProvider({children, steps}){
    ...
    return <JourneyContext.Provider value={[journeyStore, journeyActions]}>
        {children}
