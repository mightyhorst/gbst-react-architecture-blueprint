...
```js
import { Form, FormGroup, ControlLabel, InputNumeric } from 'gel-generic';
...
```js
import { FormGroup, ControlLabel, InputNumeric, InlineError } from 'gel-generic';
...
```js
import { Layouts, InputNumeric, Button } from 'gel-generic';
...
```js
import { InputNumeric, Table } from 'gel-generic';
...
<Table id="inputNumeric-table" keyField="id" data={tableInputTestData} columns={columnData} />;
```
